...
for path in (ROOT / 'content').glob('*.md'):
    ...
    meta = dict(line.split(': ', 1) for line in match[1].splitlines())
    for key in ['title', 'date', 'category', 'description']:
        if key not in meta: raise ValueError(f'Missing {key}: {path}')
    meta.update(slug=path.stem, body=markdown.markdown(match[2], extensions=['extra', 'sane_lists']), minutes=max(1, round(len(match[2])/350)))
    posts.append(meta)
posts.sort(key=lambda p: p['date'], reverse=True)
if not posts: raise ValueError('Add at least one article in content/')
...
for p in posts:
    ...
    (OUT/'posts'/f'{p["slug"]}.html').write_text(page(p['title'],p['description'],body,'../'),encoding='utf-8')
